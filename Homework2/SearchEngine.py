...
result_list = []                            #List to store the index of a quote
result_set = set()                          #Set to store the unique index of a quote
query = input("Enter your search query:")   #Reading an input string from the user
items = query.split()                       #Getting each word from the input string

#Finding the operation to perform
if('and' in query):                         #Checking for AND operation
    operation = "AND"
elif('or' in query):                        #Checking for OR operation
    operation = "OR"
else:
    operation = "AND"                       #Assuming AND operation by default

for item in items:                          #Looping through each word in the input string
    if(item!="and" and item!="or"):         #Checking if the word is not 'AND' or 'OR'
        search_set.add(item)                #Adding a word to the set

search_list = list(search_set)              #Converting list to set
print("The result of " + operation + " operation on " + str(search_list) + " is:")
for element in search_list:                 #Looping through each search word
    for i, quote in enumerate(data_list):   #Looping through each quote in the data list
        words = quote.split()               #Getting each word in a quote
        for word in words:                  #Looping through each word in a quote
            if (element == word):           #Checking if a search word matches with a quote
                result_list.append(i)       #Storing the index of the quote containing the matched word

if(operation == "AND"):                     #AND operation
    for result in result_list:              #Looping through each index stored in the result list
        if(result_list.count(result)>=len(search_list)):
            print("Found at " + str(result) + " " + data_list[result])
            while(result in result_list):   #Executing for each occurrence of an index in the list
                result_list.remove(result)  #Removing all occurrences of the index value from the list
else:                                       #OR operation
    result_set = set(result_list)           #Converting list to set to avoid duplicate results
    result_set = sorted(result_set)         #Sorting the set in ascending order
    for result in result_set:               #Looping through each element in the set
        print("Found at " + str(result) + " " + data_list[result])